...
def organize_cells_into_grid(cells):
    ...
    for row_idx, row_y in enumerate(row_positions):
        ...
        # Only add non-None values at the end of the row if there are no cells
        # with colspan
        if has_cells:
...
def process_schedule_file(file_path):
    ...
    try:
        ...
        response = {
            ...
            "visualization": {
                "output_file": (
                    f"detected_table_{os.path.splitext(os.path.basename(file_path))[0]}"
                    ".png"
                )
            },
        }
        ...
        for cell, color in zip(result["cells"], colors, strict=False):
            cv2.rectangle(
                vis_img,
...
def extract_cell_text(img, cell):
    ...
    try:
        ...
        configs = [
            # Assume uniform block of text - good for paragraphs
            "--oem 3 --psm 6",
        ...
        if not best_text:
            ...
            for config in configs:
                try:
                    ...
                    if text.strip():
                        # For direct OCR, we can't get confidence scores,
                        # so we'll use a lower threshold
        ...
        if best_text:
            ...
            for word in words:
                # Check if the word contains only I, 1, or l characters
                # (common confusion)
                if word and all(c in "I1l" for c in word):
                    # If it's a single character, use context to determine
                    # if it's I or 1
                    if len(word) == 1:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"Image size: {result['metadata']['image_size']['width']}x"
            f"{result['metadata']['image_size']['height']}"
        )
        print(
            f"Table dimensions: {result['table']['dimensions']['rows']}x"
            f"{result['table']['dimensions']['columns']}"
        )
        print("\nTable structure:")
