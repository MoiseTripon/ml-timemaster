...
class CellOCR:
    ...
    def __init__(self, rotation_confidence_threshold=70.0, minimum_confidence_threshold=50.0, 
        ...
        self.consecutive_low_confidence_threshold = 2  # Number of consecutive low confidence results to consider empty
    ...
    def _try_ocr_with_config(self, img, config):
        ...
        try:
            ...
            confidences = [
                conf for conf, word in zip(results['conf'], results['text']) 
                if str(word).strip()
            ]
            ...
            for preprocess_name, processed_img in preprocessed_images:
                for config in self.configs:
                    ...
                    if text and avg_confidence >= self.high_confidence_threshold:
                        ...
                        print(f"{cell_id}: HIGH CONFIDENCE ({avg_confidence:.1f}%) - '{best_text}'")
                    ...
                    if consecutive_low_confidence >= self.consecutive_low_confidence_threshold:
                        print(f"{cell_id}: EMPTY (2+ consecutive attempts below 50% confidence)")
                        return ""
            ...
            if best_confidence < self.rotation_confidence_threshold:
                ...
                for preprocess_name, processed_img in preprocessed_images:
                    for config in self.configs:
                        ...
                        if text and (preprocess_name not in preprocess_confidence or 
                                   avg_confidence > preprocess_confidence[preprocess_name]):
                ...
                top_preprocess = ([name for name, _ in sorted_preprocess[:3]] 
                                if sorted_preprocess else [p[0] for p in preprocessed_images[:3]])
                ...
                for preprocess_name, processed_img in preprocessed_images:
                    ...
                    for img_orient, angle in orientations:
                        for config in self.configs:
                            attempts += 1
                            text, avg_confidence = self._try_ocr_with_config(img_orient, config)
