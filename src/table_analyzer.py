...
class TableAnalyzer:
    ...
    def remove_duplicate_cells(self, cells):
        ...
        valid_cells = [
            ...
            if (
                cell["x2"] > cell["x1"]
                and cell["y2"] > cell["y1"]
                and (cell["x2"] - cell["x1"]) >= self.min_cell_size
                and (cell["y2"] - cell["y1"]) >= self.min_cell_size
            )
        ]
    ...
    def organize_cells_into_grid(self, cells):
        ...
        sorted_cells = sorted(
            cells, key=lambda c: (c["bounds"]["y1"], c["bounds"]["x1"])
        )
            ...
            for cell in sorted_cells:
                if abs(cell["bounds"]["y1"] - row_y) <= tolerance:
                    ...
                    if can_place:
                        ...
                        for r in range(
                            row_idx, min(row_idx + rowspan, len(row_positions))
                        ):
                            for c in range(
                                col_idx, min(col_idx + colspan, len(col_positions))
                            ):
                                ...
                                if r == row_idx and c > col_idx:
                                    current_row[
                                        c
                                    ] = None  # Mark covered columns as None
